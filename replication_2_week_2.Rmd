  ...
  # Create an out matrix with 8 variables, and one observation for each 'repeat', or question (length(repeats))
	...
	for(j in 1:length(repeats)){
	  # Rescale x to 0-4 then 0-1
		...
		# great to have comments like this^ throughout the code
...
# Declare variables for use in table later
 ...
 out.table %>% 
   ...
   # very nicely done table! This is great work
```
...
xtable.out.balance.text %>% 
  ...
  # very nice table again, both better than what is in the original
```
...
# Nice, glad you got some more practice with stan_glm
...
# That's fine, always good to give credit. And it may be worth learning!
```
...
Great practive to have a discussion regarding your replication. For future extensions I would recommend talking a little more about your extension. Why did you choose to do what you did? What does it show? What are the advantages/disadvantages of this method over the one's used by the author?

It may be useful in the future to have some more comments/notes from what was talked about in class where the code is doing some type of complex analysis (I know you missed some class, so nothing to worry about for this replication)
